...
Real-time은 현재 재생 배속을 반영해서 실제로 몇 분이 남았는지 영상 좌측 상단의 작은 box로 항상 보여줍니다.
...
Real-time은 사이트별 시간 표시 DOM에 의존하지 않고 현재 재생 중인 `video`를 기준으로 box를 표시합니다.
...
- 영상 좌측 상단 compact always-on box 표시
...
3. 영상 좌측 상단의 box에서 현재 배속과 실제 남은 시간을 확인합니다.
