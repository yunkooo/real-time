...
YouTube에서는 기본 시간 표시 영역에 마우스를 올리면 다음과 같은 패널이 나타납니다. Vimeo에서는 영상 하단의 작은 hover 영역에서 같은 패널을 확인할 수 있습니다.
...
- Vimeo 영상 하단 hover 영역에서 같은 패널을 보여줍니다.
...
YouTube에서는 가능하면 player의 재생 속도를 먼저 읽고, 없으면 video element의 `playbackRate`를 사용합니다. Vimeo에서는 video element의 `playbackRate`를 사용합니다. UI가 갱신되는 순간 일시적으로 `1x`처럼 보이는 것을 줄이기 위해 마지막으로 확인한 유효한 배속을 짧게 캐시합니다.
...
- `manifest.json`: Chrome MV3 메타데이터, 권한, popup, YouTube/Vimeo content script 등록 정보
- `content.js`: 플랫폼별 adapter로 활성 영상을 찾고, 실제 남은 시간을 계산하며, hover 패널을 렌더링하는 핵심 로직
- `styles.css`: 영상 페이지에 주입되는 hover 패널과 Vimeo hover 영역 스타일
...
9. Vimeo 영상과 `player.vimeo.com` embed에서 영상 하단 hover 영역에 패널이 나타나는지 확인합니다.

## 참고 사항

- 현재는 YouTube와 Vimeo를 지원하는 Chrome MV3 확장프로그램입니다.
